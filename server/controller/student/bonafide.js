...
exports.bono = async (req,res)=>{
  const {id,title,sec}=req.params;
   ...
   var bonafide_data=await bonafide.find({id:id})
    ...
    res.render('student/bonafide', { role, student, bonafide_data,notification });  
}
...
exports.certificate=async (req,res)=>{
 ...
 const section=await class_list.find({},{section_name:1});
  ...
  res.render('./student/bonafide_certificate',{ role, student,bonafide_data ,common,section,notification});
...
exports.pdf=async (req,res)=>{
  ...
  var bonafide_data=await bonafide.find({id:id})
    console.log("student:",student)
    res.redirect('./student/bonafide_certificate',{ role, student,bonafide_data ,common,section,notification});
