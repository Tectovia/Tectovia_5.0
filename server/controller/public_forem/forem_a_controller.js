...
exports.ArtsPage = async (req, res) => {

    try {
        const documents = await arts_model.find();
        ...
        res.render('public_forem/forem_arts', { vim: documents,user:req.session.user_id,role: req.session.role});
    } catch (error) {
...
// Searching Question by Arts Forem 
exports.QuestArts = async (req, res) => {
    console.log('Arts search: ', req.body.question);
    var qq = req.body.question;
    await arts_model.find({ $text: { $search: qq } }).then(data => {
         console.log('stream',data);
        res.render('public_forem/forem_arts', { vim: data,user:req.session.user_id,role: req.session.role})
    })
}


//Arts New Question Posting

exports.new_arts_question = async (req, res) => {
    ...
    try {
        ...
        const newQuestionObj = new arts_model({ question: new_Question, stream: 'arts', postedBY: {username:postedby,role:role}});
        newQuestionObj.save()
            .then(() => {
                console.log('New question posted successfully!');
                res.redirect('/forem/arts')
            })
            .catch((err) => {
...
exports.forum_arts_answer = async (req, res) => {
    ...
    try
    {
        ...
        const updatedQuestion = await arts_model.findByIdAndUpdate(objectid, { $push: { answer: { Answeredby: postedby,role:role,answer: answers,verified: false} } }, { new: true });
...
// filter Option
exports.filterArtsQuestion = async (req, res) => {
    console.log(req.body);
    const ke = req.body.AnsNO;
    try {
        let documents;
        if (ke === 'Yes') {
            documents = await arts_model.find({ answer: { $exists: true, $ne: [] } });
            documents.forEach(doc => {
                doc.answer.sort((a, b) => {
                    if (a.verified && !b.verified) return -1; // Place verified answers first
                    if (!a.verified && b.verified) return 1; // Place unverified answers last
            ...
            documents = await arts_model.find({ answer: { $size: 0 } });
        }
        res.render('public_forem/forem_arts', { vim: documents,user:req.session.user_id,role: req.session.role });
    } catch (error) {
        console.error('Error:', error);
...
exports.forem_verification_arts= async (req, res) => {
    ...
    try {
        const result = await arts_model.updateOne(
...
exports.del_question_Arts = async (req, res) => {
    console.log(req.params.id);
    const question = await arts_model.findByIdAndDelete({ _id: req.params.id })
    console.log(question);
